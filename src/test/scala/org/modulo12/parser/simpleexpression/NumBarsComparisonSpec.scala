...
class NumBarsComparisonSpec extends AnyFlatSpec with should.Matchers with Inside {
  "sql parser" should "return song correctly if num bars queried is equal to the number of bars in the song" in {
    SqlParser.parse("SELECT musicxml FROM resources where numbarlines = 2;") should equal(List("musicXMLTest.xml"))
  }

  it should "return song correctly if song's num of bars is less than requested num of bars" in {
    SqlParser.parse("SELECT musicxml FROM resources where numbarlines < 3;") should equal(List("musicXMLTest.xml"))
  }

  it should "return song correctly if song's num of bars is greater than requested num of bars" in {
    SqlParser.parse("SELECT musicxml FROM resources where numbarlines > 1;") should equal(List("musicXMLTest.xml"))
  }
  ...
  it should "return nothing if num bars is not present in the song" in {
    SqlParser.parse("SELECT midi FROM resources where numbarlines = 1;") should equal(List())
  }

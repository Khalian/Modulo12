...
class NumTracksComparsionSpec extends AnyFlatSpec with should.Matchers with Inside {
  "sql evaluator" should "return song correctly if num tracks queried is equal to the number of tracks in the song" in {
    Main.sqlEval("SELECT musicxml FROM resources where numtracks = 1;") should equal(List("musicXMLTest.xml"))
    Main.sqlEval("SELECT midi FROM resources where numtracks = 4;") should equal(List("MIDI_sample.mid"))
  }
  ...
  it should "return song correctly if song's num of tracks is greater than requested num of tracks" in {
    Main.sqlEval("SELECT musicxml FROM resources where numtracks > 0;") should equal(List("musicXMLTest.xml"))
  }
  ...
  it should "return nothing if song num tracks requested is greater than its value" in {
    Main.sqlEval("SELECT musicxml FROM resources where numtracks > 10;") should equal(List())
  }

  it should "return nothing if song num tracks requested is less than its value" in {
    Main.sqlEval("SELECT musicxml FROM resources where numtracks < 1;") should equal(List())
  }
  ...
  it should "return nothing if song num tracks requested is greater or equal to than its value" in {
    Main.sqlEval("SELECT musicxml FROM resources where numtracks >= 3;") should equal(List())
  }

  it should "return nothing if song num tracks requested is less than or equal to than its value" in {
    Main.sqlEval("SELECT musicxml FROM resources where numtracks <= 0;") should equal(List())
  }
}

...
class KeySignatureSqlSpec extends AnyFlatSpec with should.Matchers with Inside {
  ...
  it should "return song correctly if request for key signature is correct" in {
    Main.sqlEval("SELECT MUSICXML FROM resources where key = Eb AND scale = major;") should equal(List("musicXMLTest.xml"))
    Main.sqlEval("SELECT MUSICXML FROM resources where key = Eb OR scale = major;") should equal(List("musicXMLTest.xml"))
  }
